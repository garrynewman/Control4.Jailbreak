        ...
        private bool Recover(LogWindow log)
        {
            ...
            try
            {
                using (var serialPort = new SerialPort(selectedPort))
                {
                    ...
                    // Send Ctrl+D in case we are already logged in and to wake up the device
                    ...
                    if (match == null)
                    {
                        ...
                        {
                            log.WriteError("No response from the device. Ensure the device is powered on and connected.\n");
                        }
                        else 
                        {
                    ...
                    // Step 4: Generate password from MAC address
                    var password = DirectorPatch.GetDirectorRootPassword(macAddress);
                    ...
                    (match, prompt) = WaitForPrompt(serialPort, new Regex(@"^\s*0\s*$", RegexOptions.IgnoreCase | RegexOptions.Multiline));
                    if (match == null)
                    {
                        log.WriteError("failed\n\n");
                        log.WriteTrace("Console Output:\n" + prompt.Trim() + "\n");
